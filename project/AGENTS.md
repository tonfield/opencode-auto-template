...
- Job files use Protocol v2: Baseline with Protocol version, Receipts (every decision traces to a source), optional Delegation Plan for non-trivial subagent lanes, phase-grouped Progress (each phase has a pass condition), optional Subagent Receipts for accepted delegated outputs, and Closeout disclosure. Completed jobs move to `jobs/archive/`; closeout consolidates reusable lessons to `memory/`.
